...
class DataLoaderInstanceSegmentation(Dataset):
    def __init__(self, folder_path="ethz_1/A1Sample", train = True):
        super(DataLoaderInstanceSegmentation, self).__init__()
        if train:
            folder_path="ethz_1/A1Sample"
        else:     
            folder_path="ethz_1/A1_testing"
        self.train = train
        self.img_files = glob.glob(os.path.join(folder_path,"raw","*.png"))
        self.seg_mask_files = []
        self.ins_mask_files = []
